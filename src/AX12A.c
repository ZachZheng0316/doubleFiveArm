...
//设置多个舵机单字节属性
//写入成功, 返回1; 写入失败, 返回-1
int set_arm_servo_byte(char dir, int address, int value[])
{
    ...
    while(write_num-- >= 0) {
		...
		for( i = 0; i < SERVO_NUM; i++ ) {
		    dxl_set_txpacket_parameter(2 + 2 * i, base + i);
		    dxl_set_txpacket_parameter(2 + 2 * i + 1, value[i]);
		}
		dxl_set_txpacket_length((1 + 1) * 5 + 4);
...
//设置多个舵机多自己属性
//写入成功，返回1; 写入失败, 返回-1.
int set_arm_servo_word(char dir, int address, int value[])
{
    int commStatus, i, write_num = 2000;
    int base = 1;
    ...
    while(write_num-- >= 0) {
		...
		for( i = 0; i < SERVO_NUM; i++ ) {
		    dxl_set_txpacket_parameter(2 + 3 * i, base + i);
...
void wait_for_many_servo_exten(int exten[])
{
	...
	for(i = 0; i < SERVO_NUM * 2; i++)
		wait_for_one_servo_exten(i+1, exten[i]);
}
