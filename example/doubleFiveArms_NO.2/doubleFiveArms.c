...
void from_A_to_B(int num1, int num2)
{
    ...
    for(i = 0; i < SERVO_NUM/2; i++) {
        ...
        speK[i] = (int)(((double)abs(diffK[i]) + 2) * 3.5);
        if(speK[i] >= 1020)
            speK[i] = 1020;
    }
...
void hit_and_hit(int num)
{
    ...
    if(num <= 9) {
        set_one_servo_word(10, Moving_Speed, 459); //NO1:359; NO2:459
        for(i = 0; i < 10; i++) {
            posK = LKeyPos[num][4] + diffK[i];
            set_one_servo_word(10, Goal_Position, posK);
            delay_us(5 * 1000);
        }
        set_one_servo_word(10, Moving_Speed, 759); //NO1:559; NO2:659
        for(i = 8; i >= 0; i--) {
            posK = LKeyPos[num][4] + diffK[i];
            set_one_servo_word(10, Goal_Position, posK);
            delay_us(5 * 1000);
    ...
    if(num >= 10) {
        set_one_servo_word(5, Moving_Speed, 459); //NO1:359; NO2:459
        for(i = 0; i < 10; i++) {
            posK = RKeyPos[num-9][4] - diffK[i];
            set_one_servo_word(5, Goal_Position, posK);
            delay_us(5 * 1000);
        }
        set_one_servo_word(5, Moving_Speed, 759); //NO1:559; NO2:659
        for(i = 8; i >= 0; i--) {
            posK = RKeyPos[num-9][4] - diffK[i];
            set_one_servo_word(5, Goal_Position, posK);
            delay_us(5*1000);
